...
class CallTaobao {
	...
	function QueryItem( $data, $is_cached = 0 ) {
		...
		//Splitting the description to images and texts only
		if ( $Item['desc'] != '' ) {
		...
		if ( $ItemCached = $this->DBCtrl->getItemCached( $num_iid, $language_id ) ) {
			...
			$Item['desc']  = $ItemCached['description'];
		...
		elseif ( $ItemCached_CN ) {
			...
			//print_r($Item);die();
			$Item['product_description'][$language_id]['description'] = $desc.'<br>'.$images;
	...
	function addProduct( $Item ) {
		$api_path = str_replace( array( 'web_tao\item.php', 'web_tao/item.php' ), '', __FILE__ );
		//echo $api_path.'config.php'; die();
		include $api_path.'web_tao/itemsofnick.php';

		$CallTaobao2 = new CallTaobao2;
		$user_id = $CallTaobao2->get_user_id( $Item['nick'] );
		...
		$Product = array(
			...
			'taobao_nick_id'  => $user_id,
			'taobao_num_iid'  => $Item['num_iid'], //淘宝商品ID
			'taobao_url'      => $Item['detail_url'], //淘宝商品url
			'taobao_score'    => 0, //卖家信用
			'product_description'    => $Item['product_description'],
			//'product_description'    => $desc.'<br>'.$images, //description
		);

		$num_iid = $Item['num_iid'];
		$image_name = ( basename( $Item['pic_url'] ) ).'_310x310.jpg';
		$image_file = $this->root_path.'image/cache/data/'.$num_iid.'-'.$image_name;

		if ( !is_dir( $this->root_path.'image/cache/data/' ) ) {
			mkdir( $this->root_path.'image/cache/data/', 0777 );
		}
		//echo $image_file;
		if ( !file_exists( $image_file ) ) {
			@file_put_contents( $this->root_path.'image/cache/data/'.$num_iid.'-'.$image_name, file_get_contents( $Item['pic_url'].'_310x310.jpg' ) );
			$Product['image'] = 'cache/data/'.$num_iid.'-'.$image_name;
		}
		else {
			$Product['image'] = 'cache/data/'.$num_iid.'-'.$image_name;
		}
