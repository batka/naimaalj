...
class DBCtrlModel {
	...
	public function getItemCached($product_id, $language_id)
	{
		...
		$query = $this->db_api->query("SELECT * FROM {$tab_product}, {$tab_description} ".
									  "WHERE p.product_id = '{$product_id}' AND language_id = {$language_id}");
