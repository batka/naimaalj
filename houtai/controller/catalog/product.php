	...
	private $error = array(); 
  	...
  	private function getForm() {
		...
		foreach ($product_images as $product_image) {
			...
			if (substr($product_image['image'], 0, 7) == 'http://' || substr($product_image['image'], 0, 8) == 'https://') {
			
				$image = $product_image['image'];
				$parse_url = parse_url($product_image['image']);
				if (substr($parse_url['host'], -10) == 'taobao.com' || substr($parse_url['host'], -13) == 'taobaocdn.com')
				{
					$this->data['product_images'][] = array(
						'image'      => $image,
						'thumb'      => $image.'_100x100.jpg',
