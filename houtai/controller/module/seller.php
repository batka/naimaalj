...
class ControllerModuleEPHtml extends Controller {
	...
	public function index() {   
		$this->load->language('module/ep_html');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('ep_html', $this->request->post);		
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/ep_html', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		...
		if (isset($this->request->post['ep_html_module'])) {
			$this->data['modules'] = $this->request->post['ep_html_module'];
		} elseif ($this->config->get('ep_html_module')) { 
			$this->data['modules'] = $this->config->get('ep_html_module');
		}	
		...
		$this->template = 'module/ep_html.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function validate() {
		if (!$this->user->hasPermission('modify', 'module/ep_html')) {
